...
ictest-basic: rm-testcache
	cd interchaintest && go test -race -v -run TestBasicFuryaStart .

ictest-statesync: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaStateSync .

ictest-ibchooks: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaIBCHooks .

ictest-tokenfactory: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaTokenFactory .

ictest-feeshare: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaFeeShare .
...
ictest-globalfee: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaGlobalFee .

# Executes a basic chain upgrade test via interchaintest
ictest-upgrade: rm-testcache
	cd interchaintest && go test -race -v -run TestBasicFuryaUpgrade .

# Executes a basic chain upgrade locally via interchaintest after compiling a local image as furya:local
ictest-upgrade-local: local-image ictest-upgrade

# Executes IBC tests via interchaintest
ictest-ibc: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaGaiaIBCTransfer .

# Unity contract CI
ictest-unity-deploy: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaUnityContractDeploy .

ictest-unity-gov: rm-testcache
	cd interchaintest && go test -race -v -run TestFuryaUnityContractGovSubmit .

ictest-pob: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaPOB .

ictest-drip: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaDrip .

ictest-feepay: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaFeePay .

ictest-burn: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaBurnModule .

ictest-cwhooks: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaCwHooks .

ictest-clock: rm-testcache
	cd interchaintest &&  go test -race -v -run TestFuryaClock .
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c furya --local -f ./chains.yaml
endif
