...
// https://github.com/CosmosContracts/furya/v18/blob/main/cmd/osmosisd/cmd/balances_from_state_export.go
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Produce the map of address to total furya balance, both staked and UnbondingStake
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingFuryas := sdk.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingFuryas = unbondingFuryas.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingFuryas)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedFurya := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedFurya)
