...
func GetIBCHooksUserAddress(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, channel, uaddr string) string {
	// furyad q ibchooks wasm-sender channel-0 "furya1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl" --node http://localhost:26657
	cmd := []string{
		"furyad", "query", "ibchooks", "wasm-sender", channel, uaddr,
