...
function query_contract {
    ARGS=${3:-$FURYAD_COMMAND_ARGS}
    furyad query wasm contract-state smart $1 $2 --output json $ARGS
}
...
function wasm_cmd {
    ...
    tx_hash=$(furyad tx wasm execute $CONTRACT $MESSAGE $FUNDS $ARGS | jq -r '.txhash')
...
function mint_cw721 {
    ...
    TXMINT=$($BINARY tx wasm execute "$CONTRACT_ADDR" "$EXECUTED_MINT_JSON" $FURYAD_COMMAND_ARGS | jq -r '.txhash') && echo $TXMINT
}